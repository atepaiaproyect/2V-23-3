...
# HTTPRequest para verificar Firestore
@onready var http_check = $HTTPCheckPlayer
...
func _ready() -> void:
    ...
    http_check.request_completed.connect(_on_player_check_completed)
...
# --- LOGIN EXITOSO → verificar si ya tiene personaje ---
func _on_login_success(user_data: Dictionary) -> void:
    ...
    GameData.player_id    = user_data.get("localId", "")
    GameData.player_email = user_data.get("email", "")
    GameData.id_token     = user_data.get("idToken", "")
    _id_token             = GameData.id_token
    GameData.is_logged_in = true

    status_login.text = "Verificando perfil..."
...
# --- RESPUESTA DE FIRESTORE ---
func _on_player_check_completed(_result, response_code, _headers, body):
    var response = JSON.parse_string(body.get_string_from_utf8())

    if response_code == 200 and response != null and response.has("fields"):
        # Jugador YA tiene personaje → cargar datos y ir al MainHub
        var fields = response["fields"]
        GameData.player_name     = fields.get("username", {}).get("stringValue", "")
...
func _on_confirm_recover() -> void:
    ...
    status_recuperar.text = "Si el email está registrado recibirás un enlace."
